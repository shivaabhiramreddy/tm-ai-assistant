...
Endpoints:
  POST /api/method/tm_ai_assistant.exports.export_pdf   — Generate PDF from chat content
  POST /api/method/tm_ai_assistant.exports.export_excel  — Generate Excel from table data
"""
...
def export_pdf(title, content, session_id=None):
    ...
    <body>
        <div class="header">
            <span class="logo">TRUEMEAL FEEDS</span>
            <h1>{_escape_html(title)}</h1>
            <div class="subtitle">
                Generated by TM Assistant on {frappe.utils.format_datetime(frappe.utils.now_datetime(), "dd MMM yyyy, hh:mm a")}
            </div>
        </div>
        ...
        <div class="footer">
            Truemeal Feeds — AI-Generated Report | Fertile Green Industries Pvt Ltd
        </div>
    </body>
    </html>
    """
    ...
    # Save as Frappe file
    file_name = f"TM_Report_{frappe.utils.now_datetime().strftime('%Y%m%d_%H%M%S')}.pdf"
...
def export_excel(title, data, columns=None, session_id=None):
    ...
    sub_cell = ws.cell(row=2, column=1,
                       value=f"Generated by TM Assistant on {frappe.utils.now_datetime().strftime('%d %b %Y, %I:%M %p')}")
    ...
    for row_idx, row in enumerate(data, 5):
        for col_idx, col_name in enumerate(columns, 1):
            ...
            if isinstance(value, (int, float)):
                cell.number_format = number_format_inr if abs(value) >= 1000 else '0.00'
    ...
    # Save as Frappe file
    file_name = f"TM_Report_{frappe.utils.now_datetime().strftime('%Y%m%d_%H%M%S')}.xlsx"
    file_doc = frappe.get_doc({
        "doctype": "File",
        "file_name": file_name,
        "content": excel_data,
